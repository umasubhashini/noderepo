...
app.post("/generate-card/:userid", async (req, res) => {
  try {
    ...
    const selectedItem = user.selectedItems.find(
...
function mapFields(formData, section) {
  ...
  return fieldNames.map((name, idx) => {
    ...
    if (section === "contactField") {
      fieldObj.icon = formData[`${section}Icons`]
        ? formData[`${section}Icons`][idx]
        : "";
    }
...
// redirect to views/templates/invitations/flora

// invitations backend data to flora weddings template

app.get("/wedding", async (req, res) => {
  try {
    const data = await invitations.find();
    const sliders = data[0].sliders;
    const header = data[0].header;
    const footer = data[0].footer;
    const title = data[0].invitationType;
    const username = data[0].username;
    const theme = data[0].theme;
    const location = data[0].location;
    const message = data[0].message;
    const contact = data[0].contact;
    res.render("templates/invitations/flora", {
      sliders,
      header,
      footer,
      title,
...
// Assuming this is in your server-side code
app.locals.getInlineStyles = (style) =>
  ({
    "font-style-1": "font-family: Arial, sans-serif; font-size: 16px;",
    "font-style-2": "font-family: Times New Roman, serif; font-size: 18px;",
  }[style] || "");

app.locals.getInlineStylesForText = (style) =>
  ({
    "font-style-1": "color: #ff0000;", // Red color as an example
    "font-style-2": "color: #0000ff;", // Blue color as an example
  }[style] || "");
